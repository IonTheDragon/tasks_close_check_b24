	...
	implements \Bitrix\Main\Errorable, \Bitrix\Main\Engine\Contract\Controllerable
{
	...
	protected function processGroupActions()
	{
		...
		foreach ($rows as $rowId)
		{
            ...
            $task = \CTaskItem::getInstance($rowId, $this->userId);
            $data = $task->getData(false);

            list($task_children, $res) = \CTaskItem::fetchList($this->userId, [], ['PARENT_ID' => (int)$data['ID']], [], ['STATUS', 'ID']);
            if(is_array($task_children))
            {
                foreach ($task_children as $task_child) {
                    $data = $task_child->getData(false);
                    if ($data['STATUS'] != 5 && !in_array($data['ID'], $rows)) { //Если подзадача не закрыта И её нет в списке на закрытие
                        $errorsList[Loc::getMessage('TASKS_CLOSE_NOT_ALLOWED_FOR_PARENT')][] = $rowId;
                        continue 2;
                    }
                }
            }
