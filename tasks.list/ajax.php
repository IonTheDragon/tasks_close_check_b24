		...
		if (intval($_POST["id"]) > 0)
		{
			...
			else
			{
				...
				else
				{
                    ...
                    if ($_POST['mode'] === 'close')
                    {
                        list($task_children, $res_ch) = CTaskItem::fetchList(\Bitrix\Tasks\Util\User::getId(), [], ['PARENT_ID' => (int)$arTask['ID']], [], ['STATUS']);
                        if(is_array($task_children))
                        {
                            foreach ($task_children as $task_child) {
                                $data = $task_child->getData(false);
                                if ($data['STATUS'] != 5) { //Если подзадача не закрыта
                                    $jsonReply = [
                                        'status' => 'failure',
                                        'message' => Loc::getMessage('TASKS_CLOSE_NOT_ALLOWED_FOR_PARENT'),
                                    ];
                                    $APPLICATION->RestartBuffer();
                                    header('Content-Type: application/x-javascript; charset=' . LANG_CHARSET);
                                    echo CUtil::PhpToJsObject($jsonReply);
