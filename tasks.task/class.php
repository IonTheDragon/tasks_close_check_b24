	...
	implements \Bitrix\Main\Errorable, \Bitrix\Main\Engine\Contract\Controllerable
{
	...
	public function completeAction($taskId, $parameters = [])
	{
		...
		try
		{
            ...
            list($task_children, $res) = \CTaskItem::fetchList($this->userId, [], ['PARENT_ID' => (int)$data['ID']], [], ['STATUS']);
            if(is_array($task_children))
            {
                foreach ($task_children as $task_child) {
                    $data = $task_child->getData(false);
                    if ($data['STATUS'] != 5) {
                        $this->errorCollection->add('ACTION_NOT_ALLOWED.RESTRICTED', Loc::getMessage('TASKS_CLOSE_NOT_ALLOWED_FOR_PARENT'));
                        return $result;
                    }
                }
            }
		...
		try
		{
            ...
            if(is_array($task_children))
            {
                foreach ($task_children as $task_child) {
                    $data = $task_child->getData(false);
                    if ($data['STATUS'] != 5) {
                        $this->errorCollection->add('ACTION_NOT_ALLOWED.RESTRICTED', Loc::getMessage('TASKS_DELETE_NOT_ALLOWED_FOR_PARENT'));
                        return $result;
                    }
                }
            }
